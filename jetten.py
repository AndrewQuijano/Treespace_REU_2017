...
def is_tree_based(graph, name=None):
    unmatched_omnian = jetten_graph(graph, name)
...
def jetten_graph(graph, name=None):
    matched_omnians = set()
    ...
    try:
        ...
        omnians = [n for n, d in jetten.nodes(data=True) if d['biparite'] == 0]
        data = get_node_attributes(jetten, 'biparite')
        ...
        return list()
    set_minus = set(omnians) - matched_omnians
